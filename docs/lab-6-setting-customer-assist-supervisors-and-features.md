# Lab 6: Setting Customer Assist Supervisors and Features

In this lab, you will learn how assign the supervisor role to a customer assist user, and set additional features.

### Charles Holland need to supervise the members of both queues.

*Services > Customer Assist > Supervisors > Add Supervisor*

* Select Charles Holland as Supervisor
  + Select all Agents to assign to Charles Holland

### Create wrap-up reasons for agents to use.

*Services > Customer Assist > Desktop Experience > Add Wrap-up Reason*

* Name: Booking
  + All queues
* Name: Cancelation
  + All queues
* Name: Complaints
  + All queues

### Assign Operating mode to the Reservations Queue.

*Services > Customer Assist > Queue > Select Reservation Queue*

* Open call Forwarding
  + Forward calls by modes: select Emergency Closures

### Set Anita Perez to use Operating modes.

*Management > Users > Anita Perez > Calling > Call Handling > Mode Management*

* + Add Reservation Queue

| Help articles | |
| --- | --- |
| [Assign Agents to Supervisors](https://help.webex.com/en-us/article/72sb3r/Webex-Calling-Customer-Assist#task-template_5a490230-e253-468f-b5e9-73276029bd17) | [Wrap up reason and wrap up timer](https://help.webex.com/en-us/article/72sb3r/Webex-Calling-Customer-Assist#Wrap-up-reason-and-wrap-up-timer) |
| [Operating Modes](https://help.webex.com/en-us/article/fozeml/Call-routing-based-on-operating-modes-in-Webex-Calling) |  |

STOP: End of Lab 6

Raise your hand and leave it raised in the Webex Meeting. Wait for further instruction.  
![](assets/docx-image-013.png)
